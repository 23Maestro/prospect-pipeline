...
**Created**: `<REPO_ROOT>/mcp-servers/npid-native/setup-httpie-session.sh`
...
2. Creates HTTPie session at `~/.config/httpie/sessions/legacy-dashboard.example.com/prospect-id.json`
3. Tests the session against the inbox endpoint
4. Verifies authentication works

### Manual Cookie Extraction Steps

1. **Open Chrome** → `https://legacy-dashboard.example.com`
2. **Open DevTools** (Cmd+Option+I)
3. **Application tab** → Cookies → legacy-dashboard.example.com
...
http --session=prospect-id --form POST \
  https://legacy-dashboard.example.com/videoteammsg/getvideomailthreads \
  _token='YOUR_XSRF_TOKEN' thread_status=all assigned_to= search_keyword=

# 2. Get student details
http --session=prospect-id --form POST \
  https://legacy-dashboard.example.com/videoteammsg/getstudentdetails \
  _token='YOUR_XSRF_TOKEN' thread_id=12489

# 3. Assign thread
http --session=prospect-id --form POST \
  https://legacy-dashboard.example.com/videoteammsg/assignthread \
  _token='YOUR_XSRF_TOKEN' thread_id=12489 editor_id=100001 \
  stage='In Queue' status='HUDL'

# 4. Get thread messages
http --session=prospect-id GET \
  'https://legacy-dashboard.example.com/videoteammsg/getthreadmessages?thread_id=12489'
...
http --session=prospect-id --form POST \
  https://legacy-dashboard.example.com/videoteammsg/videoupdate \
...
✅ Session file exists at `~/.config/httpie/sessions/legacy-dashboard.example.com/prospect-id.json`
