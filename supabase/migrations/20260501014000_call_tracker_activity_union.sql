-- Union call_activity_events into call_tracker_events so daily
-- dial / contact activity appears alongside meeting-outcome facts.
-- call_activity_events rows are already deduped by task_id, so they
-- skip the ranked-deduplication logic applied to call_events rows.
