...
create index if not exists call_events_athlete_occurred_at_idx
  on call_events (athlete_key, occurred_at desc);

create index if not exists call_events_stage_occurred_at_idx
  on call_events (raw_crm_stage, occurred_at desc);
...
grant select, insert, update, delete on call_events to service_role;
