-- Unified call-tracker stream.
-- Clock contract:
--   occurred_at / event_at on lifecycle_meeting_set rows = lifecycle_events.created_at,
--   the time the athlete became Meeting Set and the date used for daily tracking.
--   appointment_starts_at stays in payload_json->>'starts_at' and is not the daily tracking clock.
--   sync time must not update lifecycle_events.created_at for an existing meeting_set dedupe key.
