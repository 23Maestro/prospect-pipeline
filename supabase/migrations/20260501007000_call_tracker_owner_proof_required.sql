alter table call_events
  add column if not exists owner_proof text;

create index if not exists call_events_trusted_owner_idx
  on call_events (is_tracked_owner, source_owner, owner_proof, occurred_at desc)
  where is_tracked_owner = true
    and source_owner is not null
    and owner_proof is not null;
...
select *
from classified
where tracker_outcome <> 'needs_review';
