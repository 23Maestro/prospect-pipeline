-- Carry raw CRM stage/status through call activity facts.
-- This keeps call_attempt rows tied to the CRM stage observed at ingest instead
-- of forcing reporting views and related domains to infer from task status.

alter table call_activity_events
  add column if not exists raw_crm_stage text,
  add column if not exists raw_task_status text;

update call_activity_events cae
set
  raw_crm_stage = coalesce(
    nullif(cae.raw_crm_stage, ''),
    nullif(cae.payload_json->>'raw_crm_stage', ''),
    nullif(cae.payload_json->>'selected_sales_stage', '')
  ),
  raw_task_status = coalesce(
    nullif(cae.raw_task_status, ''),
    nullif(cae.payload_json->>'raw_task_status', ''),
    nullif(cae.payload_json->>'selected_task_status', ''),
    nullif(cae.activity_subtype, ''),
    nullif(cae.activity_type, '')
  ),
  payload_json = cae.payload_json
    || jsonb_strip_nulls(
      jsonb_build_object(
        'raw_crm_stage',
        coalesce(
...
where nullif(cae.payload_json->>'selected_sales_stage', '') is not null
   or nullif(cae.payload_json->>'raw_crm_stage', '') is not null
   or nullif(cae.payload_json->>'raw_task_status', '') is not null
   or nullif(cae.raw_task_status, '') is null;

comment on column call_activity_events.raw_crm_stage is
  'Raw CRM sales stage observed when the call activity fact was ingested.';

comment on column call_activity_events.raw_task_status is
  'Raw task status observed when the call activity fact was ingested.';
...
activity_facts as (
  select
    ...
    coalesce(
      nullif(cae.raw_task_status, ''),
      nullif(cae.payload_json->>'raw_task_status', ''),
      cae.normalized_activity_status
    ) as raw_task_status,
...
select
  unified_events.*,
  case
    when unified_events.payload_json ? 'counts_as_dial' then coalesce((unified_events.payload_json->>'counts_as_dial')::boolean, false)
  ...
  case
    when unified_events.payload_json ? 'counts_as_contact' then coalesce((unified_events.payload_json->>'counts_as_contact')::boolean, false)
    when unified_events.tracker_outcome = 'meeting_set' then true
    when unified_events.raw_event_type = 'call_activity'
  ...
  case
    when unified_events.payload_json ? 'counts_as_meeting_set' then coalesce((unified_events.payload_json->>'counts_as_meeting_set')::boolean, false)
    when unified_events.tracker_outcome = 'meeting_set' then true
    else false
  end as counts_as_meeting_set,
  case
    when unified_events.payload_json ? 'counts_as_post_meeting_outcome' then coalesce((unified_events.payload_json->>'counts_as_post_meeting_outcome')::boolean, false)
