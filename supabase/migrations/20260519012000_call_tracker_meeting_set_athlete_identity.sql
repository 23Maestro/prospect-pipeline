...
dashboard_ranked as (
  select
    cte.*,
    case
      when cte.tracker_outcome = 'meeting_set'
        and coalesce(cte.counts_as_meeting_set, false)
        and coalesce(lower(nullif(cte.raw_crm_stage, '')), lower(nullif(cte.raw_task_status, '')), '') not in (
          'rescheduled',
          'reschedule pending',
          'reschedule_pending',
      ...
      then row_number() over (
        partition by coalesce(cte.athlete_key, 'event:' || cte.id::text)
        order by cte.reporting_at nulls last, cte.created_at nulls last, cte.occurred_at nulls last, cte.id
      )
      else null
    end as meeting_set_athlete_sequence
...
select
  ...
  case
    when cte.tracker_outcome = 'meeting_set' then cte.meeting_set_athlete_sequence = 1
    else cte.counts_as_meeting_set
