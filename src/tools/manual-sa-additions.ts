...
async function fetchRawSearch(term: string, includeAdminSearch: boolean): Promise<ProspectResult[]> {
  ...
  if (!response.ok) {
    const message = json?.detail || json?.message || text.slice(0, 300) || `HTTP ${response.status}`;
...
export const confirmation: Tool.Confirmation<Input> = async (input) => {
  ...
  try {
    ...
    return {
      style: candidate && stageProvided && statusProvided ? Action.Style.Regular : Action.Style.Destructive,
      message: !candidate
      ...
      info: [
        ...
        { name: "Term", value: term.slice(0, 140) },
        { name: "Selected Athlete", value: candidate ? (candidate.name || "Unknown") : candidateLabel },
        { name: "Athlete ID", value: candidate ? candidate.athlete_id : (input.athleteId || "Not provided") },
