...
import { runVideoPostUploadActions } from '../../lib/video-post-upload';
import type {
  NPIDVideoProgressPlayer,
  VideoUpdateFormValues,
...
function parseSortableHtmlDetailed(
  ...
  try {
    ...
    logVideoUpdateEvent(
      'VIDEO_UPDATES_EDIT_FORM',
      'parse-sortable-html-detailed',
      'failure',
      {},
      getErrorMessage(error),
    );
...
export type VideoUpdatesViewProps = {
  draftValues?: VideoUpdateFormValues;
        ...
        try {
          ...
          try {
            ...
            logVideoUpdateEvent('VIDEO_UPDATES_SUBMIT', 'response', 'success', {
              ...
              sortableHtmlLength: (result?.sortable_html || result?.data?.sortable_html || '')
                .length,
            });
            ...
            if (success) {
              ...
              if (AUTO_POST_UPLOAD_ACTIONS) {
                const { summary, taskHudMessage } = await runVideoPostUploadActions({
                  athleteId,
                  athleteMainId,
                  videoMsgId,
                  logEvent: logVideoUpdateEvent,
                });
                if (taskHudMessage) {
                  await showHUD(taskHudMessage);
                }
      ...
      if (values.athleteName && values.athleteName.length > 2) {
        ...
        try {
          logVideoUpdateEvent('VIDEO_UPDATES_SEARCH', 'request', 'start', {
            athleteQueryLength: values.athleteName.length,
          });
          const results = await searchVideoProgressPlayers(values.athleteName);
...
function EditSAVideosForm({
  ...
  onBack: () => void;
  selectedPlayer: NPIDVideoProgressPlayer | null;
