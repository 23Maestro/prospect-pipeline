...
window.addEventListener('popstate', () => {
  state.route = routes[window.location.pathname] ? window.location.pathname : '/set-meetings';
  void loadRoute();
});
...
async function renderSetMeetings(payload) {
  const events = (Array.isArray(payload?.events) ? payload.events : []).filter(isActualSetMeetingEvent);
  if (!events.length) {
