...
def test_parse_head_scout_slots_response_filters_and_orders_slots():
    payload = """
    [
      {"id": 1, "start": "2026-04-16T17:00", "end": "2026-04-16T18:00", "user": "Head Scout B", "title": "OPEN", "openslot": "openslot"},
      {"id": 13, "start": "2026-04-16T17:30", "end": "2026-04-16T18:00", "user": "Head Scout A", "title": "OPEN", "openslot": "openslot"},
      {"id": 2, "start": "2026-04-16T18:00", "end": "2026-04-16T19:00", "user": "Head Scout B", "title": "(NS) Noise", "openslot": "meetingset"},
      {"id": 3, "start": "2026-04-16T17:30", "end": "2026-04-16T18:00", "user": "Head Scout C", "title": "OPEN", "openslot": "openslot"},
      {"id": 14, "start": "2026-04-16T18:30", "end": "2026-04-16T19:00", "user": "Head Scout H", "title": "OPEN", "openslot": "openslot"},
      {"id": 7, "start": "16:00", "end": "17:00", "user": "Head Scout C", "title": "OPEN", "openslot": "openslot", "dow": "[1,3]"},
      {"id": 4, "start": "2026-04-16T16:30", "end": "2026-04-16T17:00", "user": "Head Scout C", "title": "Follow Up", "openslot": ""},
      {"id": 5, "start": "2026-04-16T19:00", "end": "2026-04-16T20:00", "user": "Head Scout D", "title": "OPEN", "openslot": "openslot"},
      {"id": 6, "start": "2026-04-16T20:00", "end": "2026-04-16T21:00", "user": "Head Scout F", "title": "OPEN", "openslot": "openslot"},
      {"id": 12, "start": "2026-04-17T18:00", "end": "2026-04-17T19:00", "user": "Head Scout G", "title": "OPEN", "openslot": "openslot"},
      {"id": 8, "start": "2026-04-20T17:00", "end": "2026-04-20T18:00", "user": "Head Scout D", "title": "OPEN", "openslot": "openslot"},
      {"id": 9, "start": "2026-04-12T17:00", "end": "2026-04-12T18:00", "user": "Head Scout D", "title": "OPEN", "openslot": "openslot"},
      {"id": 10, "start": "2026-04-16T17:00", "end": "2026-04-16T18:00", "user": "Head Scout B", "title": "open", "openslot": "openslot"},
      {"id": 11, "start": "2026-04-16T17:00", "end": "2026-04-16T18:00", "user": "Head Scout B", "title": "OPEN", "openslot": "openslot"}
    ]
    """
    ...
    assert david["calendar_owner_id"] == "calendar_owner_a"
    assert david["meeting_for"] == "1418020"
    assert david["city"] == "Winona"
    assert david["state"] == "MN"
    assert jeffrey["calendar_owner_id"] == "calendar_owner_b"
    assert jeffrey["meeting_for"] == "200002"
    assert luther["calendar_owner_id"] == "calendar_owner_c"
    assert luther["meeting_for"] == "200003"
    assert nasir["calendar_owner_id"] == "calendar_owner_h"
    assert nasir["meeting_for"] == "200008"
    assert nasir["city"] == "Dallas"
    assert nasir["state"] == "TX"
    assert ryan["calendar_owner_id"] == "calendar_owner_d"
    assert ryan["meeting_for"] == "200004"
    assert james["calendar_owner_id"] == "calendar_owner_e"
    assert james["meeting_for"] == "56"
    assert james["city"] == "Phoenix"
    assert james["state"] == "AZ"
    assert logan["calendar_owner_id"] == "calendar_owner_f"
    assert logan["meeting_for"] == "200006"
    assert logan["city"] == "Chandler"
    assert logan["state"] == "AZ"
    assert kenton["calendar_owner_id"] == "calendar_owner_g"
    assert kenton["meeting_for"] == "200007"
...
def test_head_scout_slots_request_preserves_selected_owner_ids_and_fields():
    ...
    assert params == [
        ...
        ("selectedowner[]", "calendar_owner_a"),
        ("selectedowner[]", "calendar_owner_b"),
        ("selectedowner[]", "calendar_owner_c"),
        ("selectedowner[]", "calendar_owner_h"),
        ("selectedowner[]", "calendar_owner_d"),
        ("selectedowner[]", "calendar_owner_e"),
        ("selectedowner[]", "calendar_owner_f"),
        ("selectedowner[]", "calendar_owner_g"),
...
def test_open_meetings_request_preserves_meetingfor_field_unchanged():
    endpoint, params = LegacyTranslator.open_meetings_to_legacy(meeting_for="200004")

    assert endpoint == "/template/template/openmeetings"
    assert params == {"meetingfor": "200004"}
...
def test_meeting_set_request_accepts_raycast_head_scout_context_fields():
    ...
    assert request.meeting_for == "200004"
    assert request.meetingfor == "200004"
    assert request.calendar_owner_id == "calendar_owner_d"
    assert request.booked_meeting_assigned_owner == "Head Scout D"
...
def test_build_head_scout_schedule_from_open_meetings_uses_concrete_slots():
    ...
    assert result["scout_name"] == "Head Scout G"
    assert result["city"] == "Prosper"
    assert result["state"] == "TX"
    assert result["calendar_owner_id"] == "calendar_owner_g"
    assert result["meeting_for"] == "200007"
...
def test_parse_head_scout_slots_response_uses_strict_monday_sunday_week_bounds():
    payload = """
    [
      {"id": 100, "start": "2026-04-26T19:00", "end": "2026-04-26T20:00", "user": "Head Scout D", "title": "OPEN", "openslot": "openslot"},
      {"id": 101, "start": "2026-04-27T18:00", "end": "2026-04-27T19:00", "user": "Head Scout D", "title": "OPEN", "openslot": "openslot"},
      {"id": 102, "start": "17:00", "end": "18:00", "user": "Head Scout D", "title": "OPEN", "openslot": "openslot", "dow": "[1,2,3,4,5]"}
    ]
    """
    ...
    current_ryan = next(scout for scout in current_week["scouts"] if scout["scout_name"] == "Head Scout D")
    next_ryan = next(scout for scout in next_week["scouts"] if scout["scout_name"] == "Head Scout D")
...
def test_parse_open_meetings_response_extracts_openeventid_and_start_time():
    ...
    assert result["slots"][0]["assigned_owner"] == "Head Scout D"
...
def test_parse_booked_meeting_response_returns_newest_match():
    payload = """
    [
      {"id": "100", "start": "2026-04-16T18:00", "end": "2026-04-16T19:00", "user": "Head Scout D", "title": "Victor Williams Football 2027 FL"},
      {"id": "200", "start": "2026-04-23T18:00", "end": "2026-04-23T19:00", "user": "Head Scout D", "title": "Victor Williams Football 2027 FL"},
      {"id": "300", "start": "2026-04-24T18:00", "end": "2026-04-24T19:00", "user": "Head Scout B", "title": "Victor Williams Football 2027 FL"}
    ]
    """
...
def test_parse_head_scout_booked_meetings_response_keeps_real_meetings_in_week():
    payload = """
    [
      {"id": "100", "start": "2026-04-21T18:00", "end": "2026-04-21T19:00", "user": "Head Scout D", "title": "Victor Williams Football 2028 TX", "openslot": "meetingset"},
      {"id": "101", "start": "2026-04-27T15:00", "end": "2026-04-27T16:00", "user": "Head Scout D", "title": "Outside Week Football 2027 MN", "openslot": "meetingset"},
      {"id": "102", "start": "2026-04-26T15:00", "end": "2026-04-26T16:00", "user": "Head Scout D", "title": "August Nyakeoga Football 2027 MN", "openslot": "meetingset"},
      {"id": "103", "start": "2026-04-24T18:00", "end": "2026-04-24T19:00", "user": "Head Scout D", "title": "OPEN", "openslot": "openslot"},
      {"id": "104", "start": "2026-04-24T18:00", "end": "2026-04-24T19:00", "user": "Head Scout F", "title": "Head Scout F Meeting Football 2027 FL", "openslot": "meetingset"}
    ]
    """
    ...
    assert [event["title"] for event in result["events"]] == [
        ...
        "Head Scout F Meeting Football 2027 FL",
...
def test_parse_athlete_events_response_reads_admin_events_title_and_description():
    ...
    assert event["assigned_owner"] == "Head Scout H"
...
def test_parse_booked_meeting_popup_response_extracts_title_and_existingtask():
    ...
    <form method="post" id="editmeetingset">
      ...
      <select name="assignedto">
        <option value="calendar_owner_d" selected>Head Scout D</option>
      </select>
    ...
    assert result["form_data"]["assignedto"] == "calendar_owner_d"
...
def test_apply_booked_meeting_description_update_preserves_title_and_sets_existingtask():
    ...
    assert result["assignedto"] == "calendar_owner_d"
