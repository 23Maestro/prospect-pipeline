...
@router.post("/create", response_model=TaskCreateCompletedResponse)
async def create_task(request: Request, payload: TaskCreateRequest):
    """
    Create a custom incomplete legacy task from the new-task popup.
    """
    session = get_session(request)
    translator = LegacyTranslator()
    contact_task = payload.contact_task or payload.athlete_id
    ...
    logger.info(
        "➕ Creating task athlete_id=%s athlete_main_id=%s contact_task=%s task_title=%s assigned_to=%s due_date=%s due_time=%s",
        payload.athlete_id,
        payload.athlete_main_id,
        contact_task,
        payload.task_title,
    ...
    try:
        ...
        created_form_data = translator.apply_task_create(
            form_data=form_data,
            athlete_id=contact_task,
            athlete_main_id=payload.athlete_main_id,
            task_title=payload.task_title,
        ...
        matching_tasks = [
            ...
            if _normalize_text(task.get("title", "")) == _normalize_text(payload.task_title)
            and _normalize_text(task.get("description", "")) == _normalize_text(payload.description)
            and not str(task.get("completion_date") or "").strip()
        ]
        task_id = str(matching_tasks[0].get("task_id") or "").strip() if matching_tasks else None
