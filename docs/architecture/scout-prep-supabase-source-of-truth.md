...
| Pending Clients | Appointment post-meeting outcome truth | Reads `appointments.post_meeting_result` for follow-up, reschedule-pending, no-show, and canceled outcomes | Pending Clients domain; `pending_client_watchlist` is support/tombstone state only | Confirmation cache may support meeting identity/timing context, but it is not lifecycle truth. Pending Clients admission must not require a `pending_client_watchlist` row. Scheduled sync/repair may backfill support rows, but appointment outcome truth remains the gate. |
