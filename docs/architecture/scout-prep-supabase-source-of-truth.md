...
- `athlete_lifecycle_current`, `athlete_lifecycle_timeline`, `active_athlete_meeting_truth`, `call_tracker_*`, `weekly_operator_funnel_metrics`, `meeting_truth_anomalies`, `meeting_events`, and `call_activity_events` have been purged as views/tables where they existed live. `athlete_pipeline_state` and `reminders` remain migration/delete targets.
