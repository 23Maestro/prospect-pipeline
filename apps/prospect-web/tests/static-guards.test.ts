...
test('call tracker public contract documents count flags as the reporting source of truth', () => {
  ...
  assert.equal(contract.liveSupabaseApi.workflowCron, null);
...
test('call tracker archive selector is backed by existing weekly-results files', () => {
  ...
  assert.match(stylesText, /--command-bg:[\s\S]*linear-gradient\(135deg, #01040b 0%, #081120 46%, #02050b 100%\)/);
  assert.match(stylesText, /@media \(prefers-color-scheme: light\)/);
  assert.match(stylesText, /body \{[\s\S]*background: var\(--command-bg\);/);
  ...
  assert.match(appText, /isScheduledConfirmationTaskMeetingArtifact/);
  assert.match(appText, /return row\.counts_as_meeting_set === true && !isScheduledConfirmationTaskMeetingArtifact\(row\)/);
  assert.match(appText, /if \(details\) \{[\s\S]*\.\.\.metricsFromRows\(scopedActivityRows\(\)\)/);
  ...
  const juneArchive = index.weeks.find((week: { file?: string }) => week.file === '2026-06-08_to_2026-06-12.json');
  assert.equal(juneArchive?.meetingsSet, 17);
  assert.equal(juneArchive?.setRate, 77);
...
test('meetings readback page is live, read-only, and server-backed', () => {
  ...
  assert.match(stylesText, /@media \(prefers-color-scheme: light\) \{[\s\S]*\.status-chip\.set \{[\s\S]*color: #0057bd;/);
  assert.match(stylesText, /@media \(prefers-color-scheme: light\) \{[\s\S]*\.status-chip\.bad \{[\s\S]*color: #b91c1c;/);
  assert.match(stylesText, /@media \(prefers-color-scheme: light\) \{[\s\S]*\.status-chip\.won \{[\s\S]*color: #04783a;/);
...
test('prospect mobile exposes set meetings, scout schedules, and contact search tabs', () => {
  ...
  assert.match(pageText, /meta name="theme-color" media="\(prefers-color-scheme: light\)" content="#f6f9ff"/);
  assert.match(pageText, /meta name="theme-color" media="\(prefers-color-scheme: dark\)" content="#000000"/);
  assert.match(pageText, /meta name="color-scheme" content="light dark"/);
  assert.match(pageText, /styles\.css\?v=20260615-title-contrast/);
  assert.match(stylesText, /color-scheme: light dark/);
  assert.match(stylesText, /@media \(prefers-color-scheme: light\)/);
  assert.match(stylesText, /--page-bg:[\s\S]*linear-gradient\(145deg, #000 0%, #030712 46%, #010817 100%\)/);
  assert.match(stylesText, /--page-bg:[\s\S]*linear-gradient\(145deg, #f8fbff 0%, #edf5ff 48%, #f9fbff 100%\)/);
  assert.match(stylesText, /\.row,[\s\S]*\.form-panel \{[\s\S]*background: var\(--glass-card\);/);
  assert.match(stylesText, /\.row,[\s\S]*\.form-panel \{[\s\S]*box-shadow: var\(--card-shadow\);/);
  assert.match(stylesText, /\.row:nth-of-type\(2n\) \{[\s\S]*background: var\(--glass-card-alt\);/);
  assert.match(stylesText, /\.modal-panel \{[\s\S]*background: var\(--modal-bg\);/);
  assert.match(stylesText, /\.field input,[\s\S]*\.field textarea \{[\s\S]*background: var\(--field-bg\);/);
  assert.match(stylesText, /\.mobile-toast \{[\s\S]*background: var\(--toast-bg\);/);
  assert.match(stylesText, /\.reschedule-button,[\s\S]*\.reschedule-search-button \{[\s\S]*#f59e0b/);
  assert.match(stylesText, /\.topbar \{[\s\S]*border: 0;[\s\S]*background: transparent;[\s\S]*box-shadow: none;/);
  ...
  assert.match(stylesText, /--brand-title: #fff;/);
  assert.match(stylesText, /--brand-title: #101828;/);
...
test('prospect mobile scout schedules use short cache and modal schedule actions', () => {
  ...
  assert.match(appText, /data-reschedule-week-toggle/);
  assert.match(appText, /loadRescheduleSchedulePayload\(nextWeek\)/);
  assert.match(appText, /showLoadingModal\('Loading slots'\)/);
  assert.match(appText, /data-loading-modal/);
  assert.match(appText, /admin-modal-button/);
  assert.match(stylesText, /\.loading-backdrop \{/);
  assert.match(stylesText, /\.loading-spinner \{/);
  assert.match(stylesText, /\.reschedule-scout-row \{[\s\S]*grid-template-columns: minmax\(0, 1fr\) auto/);
...
test('home command center uses the reduced glowing shield svg', () => {
  ...
  assert.match(stylesText, /color-scheme: light dark/);
  assert.match(stylesText, /--home-bg: #000;/);
  assert.match(stylesText, /@media \(prefers-color-scheme: light\)/);
  assert.match(stylesText, /--home-title: #080f1f;/);
  assert.match(stylesText, /--home-brand-text: #101828;/);
  assert.match(stylesText, /--home-scout-bg: linear-gradient\(180deg, #1887ff, #0066d8\)/);
  assert.match(stylesText, /\.home-shell \{[\s\S]*background: var\(--home-shell-bg\);/);
  assert.match(stylesText, /\.home-copy h1 \{[\s\S]*color: var\(--home-title\);/);
  assert.match(stylesText, /\.home-panel \{[\s\S]*background: var\(--home-panel-bg\);/);
  assert.match(stylesText, /\.home-surface-calls \{[\s\S]*background: var\(--home-calls-bg\);/);
  assert.match(stylesText, /\.home-surface-scout \{[\s\S]*background: var\(--home-scout-bg\);/);
  assert.match(stylesText, /\.home-surface-scout strong \{[\s\S]*color: #fff;/);
  assert.match(stylesText, /\.home-surface-meetings \{[\s\S]*background: var\(--home-meetings-bg\);/);
